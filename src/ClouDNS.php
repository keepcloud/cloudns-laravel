...
class ClouDNS
{
    ...
    {
        $this->apiUrl = config('cloudns.api_url');
        $this->id = config('cloudns.id');
        $this->password = config('cloudns.password');
        $this->user_type = $this->determineUserType($this->id);
    }
    private function determineUserType($auth_id)
    {
        return is_numeric($auth_id) ? 'sub-auth-id' : 'sub-auth-user';
    }
    private function apiRequest($api_data, $api_url)
    {
        ...
        curl_setopt($init, CURLOPT_URL, $this->apiUrl . $api_url . '.json');
